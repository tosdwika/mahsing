...
@section('content')
    ...
    <section class="content">
            ...
            <div class="col-xs-12">
                ...
                <div class="box">
                    <div class="box-header">
                        ...
                        <button class="pull-right btn btn-primary btn-sm" data-toggle="modal" data-target="#new-modal"><i
                                class="fa fa-plus"></i> New Order</button>
                        <a class="pull-right btn btn-primary btn-sm" href="/download/master_upload_iami.xlsx">IAMI Master
                            Template</a>
                    </div>
                    <!-- /.box-header -->
                    <div class="box-body">
                                ...
                                @foreach ($list as $lists)
                                    <tr>
                                        <td style="vertical-align: middle"><a
                                                href="{{ route('iami.order') }}/{{ $lists->id }}">{{ $lists->order_number }}</a>
                                        </td>
                                        <td style="vertical-align: middle; text-align: center;">{{ $lists->order_date }}
                                        </td>
                                        <td style="vertical-align: middle; text-align: center;">
                                            {{ $lists->created_by == Auth()->user()->id ? Auth()->user()->name : Auth()->user()->name }}
                                        </td>
                                        <td style="vertical-align: middle; text-align: center;">
                                            <div
                                                class="progress progress-md @if (number_format((float) (($lists->totalall - $lists->totalnull) * 100) / $lists->totalall, 2, '.', ',') != 100) active @endif">
                                                <div class="progress-bar @if (number_format((float) (($lists->totalall - $lists->totalnull) * 100) / $lists->totalall, 2, '.', ',') == 100) progress-bar-success @elseif(number_format((float) (($lists->totalall - $lists->totalnull) * 100) / $lists->totalall, 2, '.', ',') > 0) progress-bar-warning progress-bar-striped @else progress-bar-danger progress-bar-striped @endif"
                                                    style="width: {{ number_format((float) (($lists->totalall - $lists->totalnull) * 100) / $lists->totalall, 2, '.', ',') }}%">
                                                </div>
                                            </div>
                                        </td>
                                        <td style="vertical-align: middle; text-align: center;"><span
                                                class="badge @if (number_format((float) (($lists->totalall - $lists->totalnull) * 100) / $lists->totalall, 2, '.', ',') == 100) bg-green @elseif(number_format((float) (($lists->totalall - $lists->totalnull) * 100) / $lists->totalall, 2, '.', ',') > 0) bg-yellow @else bg-red @endif">{{ number_format((float) (($lists->totalall - $lists->totalnull) * 100) / $lists->totalall, 2, '.', ',') }}</span>
                                        </td>
                                        <td style="vertical-align: middle; text-align: center;">
                                            <button type="button" class="btn-delete btn btn-xs btn-danger"
                                                data-toggle='modal' data-file="{{ $lists->id }}"
                                                data-target="#modal-delete"><i class="fa fa-trash"></i> Delete</button>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
        ...
        <div class="modal fade" id="new-modal">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <form id="inject" method="post" action="{{ route('iami.sendorder') }}" class="form-horizontal"
                        ...
                        <div class="modal-body">
                            <div class="row">
                                <div class="col-xs-12">
                                    <small>Mode pengisian :</small>
                                </div>
                                <div class="col-xs-2">
                                    <div class="radio">
                                        <label>
                                            <input type="radio" name="choice" value="file" checked>
                                            Import
                                        </label>
                                    </div>
                                </div>
                            </div><br>
                            <div id="file_opt" class="row">
                                <div class="col-xs-12">
                                    <input type="file" id="file" name="file" class="form-control">
                                    <small>File yang diunggah harus file Excel yang copy tanpa ada password dari File
                                        IAMI
                                        Master Template Original.</small>
                                </div>
                            </div>
                        </div>
        ...
        <div class="modal fade" id="modal-delete-confirmation">
            <div class="modal-dialog">
                <div class="modal-content">
                    <form id="delete_form" method="post" action="{{ route('iami.forcedelete.process') }}"
                        ...
                        <div class="modal-body">
                            Mohon isi form berikut untuk menghapus data order <span id="data_delete_confirmation"
                                style="font-weight: bold;"></span>
                            <div id="data_input"></div>
                            <div class="form-group row">
                                <div class="col-md-6">
                                    <label>* Dilakukan oleh : </label>
                                    <input class="form-control" type="text" name="request_by" onkeyup="filltext();"
                                        placeholder="Nama Pemohon" value="{{ Auth::user()->name }}" readonly>
                                </div>
                                <div class="col-md-6">
                                    <label>* Disetujui oleh : </label>
                                    <input class="form-control" type="text" name="approved_by" onkeyup="filltext();"
                                        placeholder="Yang menyetujui">
                                </div>
                            </div>
                            <textarea class="form-control" name="reason" rows="4" form="delete_form" placeholder="* Alasan..."
                                onkeyup="filltext();"></textarea>
                            <small style="font-weight: bold;">*) Semua form harus terisi.</small><br>
                            <small style="font-weight: bold;">Catatan:</small>
                            <small>Tindakan ini akan menghapus data order dan label. Data yang dihapus, tidak bisa
                                dikembalikan.</small>
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-default pull-left" data-dismiss="modal">Close</button>
                            <button type="submit" class="btn btn-danger" disabled="true"
                                id="delete_confirmation_btn">Delete</button>
                        </div>
                    </form>
                </div>
                <!-- /.modal-content -->
    ...
    <script>
        ...
        $('#new-modal').on('click', 'input[type="radio"]', function(event) {
            ...
            switch (option) {
                case "form":
                    ...
                    $("#form_opt").find('input[name="order_number"]').attr('required', true);
                    $("#form_opt").find('input[name="purchase_order_number"]').attr('required', true);
                    $("#form_opt").find('input[name="order_date"]').attr('required', true);
                    $("#form_opt").find('input[name="order_time"]').attr('required', true);
                    $("#form_opt").find('input[name="delivery_cycle"]').attr('required', true);
        ...
        $("#part_list").on("click", "input[type='checkbox']", function(event) {
            ...
            var record = {
                ...
                'part_number': row.find('td:eq(1)').text(),
                'part_desc': row.find('td:eq(2)').text(),
                'supplier_data': row.find('td:eq(3)').text(),
                'box_type': row.find('td:eq(4)').text(),
                'qty_box': row.find('td:eq(5)').text()
            }
        ...
        $('#btn-add').click(function() {
            ...
            $.each(part_list, function(key, value) {
                ...
                $('#form_list').append('<tr>' +
                    ...
                    '<td style="width: 15%;"><input class="form-control input-sm" type="text" name="part_num[]" value="' +
                    value['part_number'] + '" readonly></td>' +
                    '<td><input class="form-control input-sm" type="text" name="part_desc[]" value="' +
                    value['part_desc'] + '" readonly></td>' +
                    '<td style="width: 10%;"><input class="form-control input-sm" type="text" name="supplier_data[]" ' +
                    supplier_data + '></td>' +
                    '<td style="width: 10%;"><input class="form-control input-sm" type="text" name="uniq_no[]" required></td>' +
                    '<td style="width: 10%;"><input class="form-control input-sm" type="text" name="box_type[]" ' +
                    box_type + '></td>' +
                    '<td style="width: 10%;"><input class="form-control input-sm" type="number" style="text-align: right;" name="qty_box[]" value="' +
                    value['qty_box'] + '" readonly></td>' +
        ...
        $('#delete_btn').click(function() {
            var target = $(this).data('file');
            $.ajax({
                url: '/iami/order/delete/' + target,
                type: 'GET',
                dataType: "json",
                beforeSend: function(e) {
                    // consolo.log('data masuk')
