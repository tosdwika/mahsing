...
use Excel;
use PhpOffice\PhpSpreadsheet\Writer\Pdf as PDF;
...
class OrderController extends Controller
{
    ...
    public function index()
    {
        ...
        $list = Order::whereBetween('iami_orders.created_at', [$get_date, $date_now])
            ...
            ->select('iami_orders.number', 'iami_labels.msi_label_scan_at', 'iami_orders.id', 
            'iami_orders.order_number', 'iami_orders.order_date', 'iami_orders.order_time', 'users.name',
            DB::raw('count(IF(iami_labels.msi_label_scan_at is NULL,1,NULL)) as totalnull'),
            DB::raw('count(iami_order_lists.total_kanban) as totalall'))
            ->orderBy('iami_orders.id', 'asc')
            ->groupBy('iami_orders.number', 'iami_labels.msi_label_scan_at', 'iami_orders.id', 
            'iami_orders.order_number', 'iami_orders.order_date', 'iami_orders.order_time', 'users.name')
            ->get();
           ...
           if ($request->hasFile('file')) {
                ...
                if (!empty($datas) && $datas->count() > 0) {
                    if (!empty($datas)) {
                        ...
                        $order_date = Carbon::createFromTimestamp( ($first_data['order_date'] - 25569) * 86400 );
                        $order_timex = Carbon::createFromTime((int)($first_data['order_time'] * 24), 0, 0);
                        $order_time = $order_timex->toTimeString();                        
